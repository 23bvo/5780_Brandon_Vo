...
void PI_update(void) {
    ...
    // 1. Calculate Error Signal
    // target_rpm and motor_speed should be in the same units. 
    // If target_rpm is already scaled to match encoder counts, use:
    error = target_rpm - motor_speed;

    // 2. Calculate Integral portion (Accumulation)
    // The integral is the sum of errors over time multiplied by the integral gain.
    error_integral += (Ki * error);

    // 3. Clamp the Integral (Anti-Windup)
    // This prevents the controller from "over-reacting" if the motor is stalled or blocked.
    ...
    // 4. Calculate Proportional portion and Total Output
    // Output = (Proportional Gain * current error) + accumulated integral
    int32_t raw_output = (Kp * error) + error_integral;

    // 5. Scale output to Duty Cycle range (0-100)
    // Right-shifting by 5 is equivalent to dividing by 32 (3200 / 32 = 100).
    int16_t output = (int16_t)(raw_output >> 5);

    // 6. Final Output Clamping
    // Ensure the duty cycle never exceeds the bounds of the PWM timer.
    if (output > 100) {
        output = 100;
    } else if (output < 0) {
        output = 0;
    ...
    duty_cycle = (uint8_t)output; // For RTT/STMStudio logging

    // 7. Monitor Motor Current
